...
class apr03_EoS(object):
    def __init__ (self,N_apr03
        ...
        #self.find_LDP_HDP_apr03_intersect_and_make_full_apr03_EoS()
    ...
    #This function retruns 0 when total charge density is zero for a given rho and x_p
    ...
    #Find x_p in nuclear matter phase
...
class CFL_EoS(object):
    ...
    def chemical_potential_electron_CFL_with_kaons_functional_test(self,mu_e_x_p_rho,mu_q,pF):
    ...
    def chemical_potential_electron_CFL_with_kaons_test(self,mu_q,pF,mu_e_x_p_rho_guess):
        mu_e_x_p_rho,info,status,message=(
        optimize.fsolve(self.chemical_potential_electron_CFL_with_kaons_functional_test,mu_e_x_p_rho_guess,args=(mu_q,pF),full_output = True))
    ...
    def build_CFL_with_kaons_EoS(self):
        ...
        for i in range(self.N_CFL_kaons):
            ...
            if(end!=True): 
                mu_e_x_p_rho_NM,status = self.chemical_potential_electron_CFL_with_kaons_test(mu_q,pF,mu_e_x_p_rho_guess)
    ...
    def find_transition_mu_q_to_mix_phase(self,mu_q_guess):
        for i in range(10):
            mu_q_intersect,info,status,message = optimize.fsolve(self.find_transition_mu_q_to_mix_phase_functional,mu_q_guess,full_output =
                    True)
            if(status!=1):
                mu_q_guess = [mu_q_guess[0]+random.randrange(-10,10)/100]
            else:
                break
        if(i==9):
    ...
    def full_eos(self):
        ...
        self.P_of_mu_q_vec = np.zeros(self.N)
        self.e_of_mu_q_vec = np.zeros(self.N)
        self.rho_of_mu_q_vec = np.zeros(self.N)
        
        
        if(self.status == "Fail"):
            self.P_of_mu_q_vec = self.eos_apr03.P_apr03_combined_vec
            self.e_of_mu_q_vec = self.eos_apr03.e_apr03_combined_vec
            self.rho_of_mu_q_vec = self.eos_apr03.rho_apr03_combined_vec
        ...
        for i in range(self.N): 
            if(NM==True):
                ...
                if(mu_q <= self.mu_q_transition):
                    self.P_of_mu_q_vec[i] = P_NM
                    self.mu_q_vec[i] = mu_q
                    self.rho_of_mu_q_vec[i] = np.interp(P_NM,self.eos_apr03.P_apr03_combined_vec,self.eos_apr03.rho_apr03_combined_vec)
                    self.e_of_mu_q_vec[i] = np.interp(P_NM,self.eos_apr03.P_apr03_combined_vec,self.eos_apr03.e_apr03_combined_vec)
                else:
                    ...
                    self.P_of_mu_q_vec[i] = P_mix
                    self.rho_of_mu_q_vec[i] = self.rho_mixed_phase_vec[self.mix_index_start+i_mix]
                    self.e_of_mu_q_vec[i] = self.e_mixed_phase_vec[self.mix_index_start+i_mix]
                    i_mix+=1
            elif(MIX == True):
                    self.mu_q_vec[i] = self.mu_q_CFL_with_kaons_vec[self.mix_index_start+i_mix]
                    P_mix = self.P_CFL_with_kaons_vec[self.mix_index_start+i_mix]
