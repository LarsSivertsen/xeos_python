...
class EOS_set:
    ...
    def p_QHD_for_kaon_phase(self,dens,Yp,kFe,mu_n,mu_p,*fields):
        """
        Pressure of neutron star matter. Returns P in MeV^4
        """
        ...
        Mp = self.m-s_field-.5*d_field #proton effective mass
        Mn = self.m-s_field+.5*d_field #neutron effective mass


        if(mu_n-v_field+.5*p_field>Mn):
            kF_n = np.sqrt((mu_n-v_field+.5*p_field)**2-Mn**2)
        else:
            kF_n = 0

        if(mu_p-v_field-.5*p_field>Mp):
            kF_p = np.sqrt((mu_p-v_field-.5*p_field)**2-Mp**2)
        else:
            kF_p=0
        kF_e = kFe
        kF_m = (kF_p**3-kF_e**3)**(1/3)
