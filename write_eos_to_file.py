...
def write_EoS_to_file(eos,filename):
    ...
    file.write("e[MeV/fm^3], P[MeV/fm^3], rho[fm^-3], v2[1]\n")
    try:
        e_vec = eos.e_w_crust_vec
        P_vec = eos.P_w_crust_vec
        rho_vec = eos.rho_w_crust_vec
        v2_vec = v2_w_crust_vec
    except:
        e_vec = eos.e_vec
        P_vec = eos.P_vec
        rho_vec = eos.rho_vec
    ...
    for i in range(len(e_vec)):
        ...
        v2 = v2_vec[i]
        if(P!=0):
            file.write(str(e)+" "+str(P)+" "+str(rho)+" "+str(v2)+"\n")
    file.close()
...
def read_EoS_from_file(filename):
    ...
    for line in file:
        ...
        v2_vec.append(v2)
    file.close()
    return np.array(P_vec),np.array(e_vec),np.array(rho_vec),np.array(v2_vec)
